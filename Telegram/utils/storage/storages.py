...
from typing import List, Tuple, Union, Dict, Optional, AnyStr
...
class PGStorage(BaseStorage):
    ...
    async def get_db(self) -> asyncpg.Connection:
        ...
        await self._db.execute("""CREATE TABLE IF NOT EXISTS "aiogram_states"(
                                        ...
                                        "state" TEXT NOT NULL)""")
        await self._db.execute("""CREATE TABLE IF NOT EXISTS "aiogram_datas"(
                                    "user" BIGINT NOT NULL PRIMARY KEY,
                                    "chat" BIGINT NOT NULL,
                                    "data" JSON)""")
        await self._db.execute("""CREATE TABLE IF NOT EXISTS "aiogram_buckets"(
                                    "user" BIGINT NOT NULL PRIMARY KEY,
                                    "chat" BIGINT NOT NULL,
    ...
    async def set_state(self, *, chat: Union[str, int, None] = None,
        ...
        if state is not None:
            ...
            await db.execute("""DELETE FROM "aiogram_states" WHERE chat=$1 AND "user"=$2""", chat, user)

    async def get_state(self, *, chat: Union[str, int, None] = None, user: Union[str, int, None] = None,
                        default: Optional[str] = None) -> Optional[str]:
        chat, user = map(int, self.check_address(chat=chat, user=user))
        db = await self.get_db()
        result = await db.fetchval("""SELECT "state" FROM "aiogram_states" WHERE "chat"=$1 AND "user"=$2""", chat, user)
    ...
    async def set_data(self, *, chat: Union[str, int, None] = None, user: Union[str, int, None] = None,
        ...
        db = await self.get_db()
        if data:
            await db.execute("""INSERT INTO "aiogram_datas" VALUES($1, $2, $3)"""
                             """ON CONFLICT ("user") DO UPDATE SET "data" = $3""",
                             user, chat, jsonpickle.encode(data))
        else:
            await db.execute("""DELETE FROM "aiogram_datas" WHERE "chat"=$1 AND "user"=$2""", chat, user)

    async def get_data(self, *, chat: Union[str, int, None] = None, user: Union[str, int, None] = None,
                       default: Optional[dict] = None) -> Dict:
        chat, user = map(int, self.check_address(chat=chat, user=user))
        db = await self.get_db()
        result = await db.fetchval("""SELECT "data" FROM "aiogram_datas" WHERE "chat"=$1 AND "user"=$2""", chat, user)
    ...
    async def get_bucket(self, *, chat: Union[str, int, None] = None, user: Union[str, int, None] = None,
        ...
        chat, user = map(int, self.check_address(chat=chat, user=user))
        db = await self.get_db()
        result = await db.fetchval("""SELECT "bucket" FROM "aiogram_buckets" WHERE "chat"=$1 AND "user"=$2""", chat, user)
        return jsonpickle.decode(result) if result else default or {}
    ...
    async def set_bucket(self, *, chat: Union[str, int, None] = None, user: Union[str, int, None] = None,
        ...
        chat, user = map(int, self.check_address(chat=chat, user=user))
        db = await self.get_db()
        await db.execute("""INSERT INTO "aiogram_buckets" VALUES($1, $2, $3)"""
    ...
    async def reset_all(self, full=True):
        db = await self.get_db()
        await db.execute("DROP TABLE aiogram_states")
        if full:
            await db.execute("DROP TABLE aiogram_datas")
            await db.execute("DROP TABLE aiogram_buckets")

    async def get_states_list(self) -> List[Tuple[int, int]]:
        db = await self.get_db()
        items = await db.fetch("SELECT * FROM aiogram_states")
